...
\end{figure}
  ...
  Nous ne supportons pour le moment que les opérations sur les entiers signés ($\mathds{Z}$), donc pas d'opérateurs ``non signés'' (sur $\mathds{N}$), ou d'opérateurs logiques (sur \raisebox{0.3ex}{$\mathds{Z}$}$/$\raisebox{-0.3ex}{$2\mathds{Z}$}).
...
\end{itemize}
  ...
  \ccc{Les scénarios où on ne peut pas identifier une valeur constante pour le nombre de bits à décaler dans le cas d'une instruction
...
%TOLDO: parler des conjonctions de prédicats avant
  ...
  \subsubsection{Algorithme complet naïf ($\theta(n!)$)}

...
\begin{document}
  ...
  Pour le choix de notre solveur SMT, nous avons dressé un petit état de l'art en nous basant principalement sur des résultats de la SMT-COMP~\cite{SMT-COMP}, une compétition entre solveurs, en éliminant d'office les solveurs qui ne présentaient pas d'API utilisable en C++ puisque le framework OTAWA est en C++ :
...
\end{figure}
  ...
  Nous nous désintéressons complètement au fonctionnement de cette traduction, qui est expliqué plus en détail dans cet article de H. Cassé, F. Birée et P. Sainrat~\cite{otawa2}, puisque nous ne travaillerons dans le cadre de notre analyse qu'avec les instructions sémantiques fournies par OTAWA.
...
% De plus, un ensemble de prédicats insatisfiable, Ps donnerait un ensemble d'état vide { } montrant par là qu'il n'existe aucun chemin d'exécution permettant d'atteindre ce point du programme.
  ...
  Nous allons maintenant examiner le fonctionnement de l'algorithme d'analyse, et les structures de données qu'il manipule.
...
%OKHUG: pas de représentation du ET, OU, NON logiques ? Expliquer pourquoi.
  ...
  on pourra en déduire que le chemin qui passe par les arcs $\{1\rightarrow 2, 3\rightarrow 5\}$ est infaisable (par transitivité du ``$=$'').
...
\end{itemize}
  ...
  % TODO: on a oublie un cas, a changer dans le code aussi !!! (ADD et SUB)
  \begin{lstlisting}[mathescape]
    t [ADD d a b] p =
        if (d = a) then
	    if (d = b) then (* d <- d+d *)
		(* on rajoute un predicat pour dire que d est pair *)
		(d % 2 = 0)
		@ (* et on remplace d par d/2 *)
		(update d (d/2) p)
	    else (* d <- d+b *)
		(update d (d-b) p)
        else 
...
%TOLDO: parler des conjonctions de prédicats avant
  ...
  Nous allons noter, dans cet algorithme et dans les suivants $p_i^{c_i}$ le prédicat annoté par l'arc $c_i$.
  ...
  On a donc obtenu l'information ``$\{c_1, c_2\}$ et $\{c_1, c_4\}$'' sont des chemins infaisables, au lieu de ``$\{c_1, c_2, c_3, c_4\}$'' est un chemin infaisable. C'est beaucoup mieux : on exclut plus de chemins (3 en l'occurence).
  ...
  Il est à noter que l'algorithme a été simplifié puisqu'en réalité il ne manipule pas exactement des prédicats annotés par des arcs $c_i$, mais plutôt des \textit{listes} d'arcs $c_i$, afin de traiter les cas où plusieurs parties du programme sont responsables de la génération de ce prédicat. Le principe reste toutefois le même : pour une liste de prédicats $\{p_1^{c_1}, p_2^{c_2}, ..., p_n^{c_n}\}$ formant une conjonction insatisfiable, le chemin infaisable est $c_1 \cup c_2 \cup ... \cup c_n$.
...
\begin{thebibliography}{1}
    ...
    \bibitem{otawa}Hugues Cassé, Christine Rochange, Pascal Sainrat. An Open Framework for
    WCET Analysis. Dans : IEEE Real-Time Systems Symposium - WIP
    session, Lisbonne, 05/12/2004-07/12/2004, IEEE, p. 13-16, décembre 2004.
    \bibitem{cousot}Patrick Cousot and Rahida Cousot. 1992. \textit{Abstract interpretation and application to logic programs}. J. Log. Program. 13, 2-3 (July 1992), 103-179. \url{http://dx.doi.org/10.1016/0743-1066(92)90030-7}
    ...
    \bibitem{entcs}\textit{Electronic Notes in Theoretical Computer Science (Entcs)}. Elsevier Sci. Pub. B. V., Amsterdam, The Netherlands, The Netherlands. \url{http://dx.doi.org/10.1016/j.entcs.2010.09.014}
    ...
    \bibitem{otawa2}Hugues Cassé, Florian Birée, Pascal Sainrat. Multi-architecture Value Analysis for Machine Code (regular paper). Dans / In : Workshop on Worst-Case Execution Time Analysis, Paris, 09/07/2013, (Eds.), OASICs, Dagstuhl Publishing, p. 42-52, juillet / july 2013.

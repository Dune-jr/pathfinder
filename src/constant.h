...
public:
	Constant();
	Constant(t::int32 val, bool relative);
	Constant(t::int32 val, constant_kind_t _kind = CONSTANT_ABSOLUTE);
	...
	Constant& operator+=(const Constant& c);
	Constant& operator-=(const Constant& c);
	inline Constant operator+(t::int32 val) const { return *this + Constant(val); }
	inline Constant operator-(t::int32 val) const { return *this - Constant(val); }
	inline Constant operator*(t::int32 val) const { return *this * Constant(val); }
	inline Constant operator/(t::int32 val) const { return *this / Constant(val); }
	inline Constant operator%(t::int32 val) const { return *this % Constant(val); }
	inline Constant& operator+=(t::int32 val) { return (*this += Constant(val)); }
	inline Constant& operator-=(t::int32 val) { return (*this -= Constant(val)); }

...
\frame{\frametitle{Les instructions sémantiques d'OTAWA}
    ...
    \begin{itemize}[<+->]
	\item indicatrices du flot du programme : \texttt{BRANCH}, \texttt{TRAP}, \texttt{CONT}
	\item d'invalidation : \texttt{SCRATCH}
	\item d'assignation de valeur : \texttt{SET}, \texttt{SETI}
	\item arithmétiques : \texttt{ADD}, \texttt{SUB}, \texttt{MUL}, \texttt{DIV}, \texttt{MOD}
	\item \sout{logiques : \texttt{NOT}, \texttt{AND}, \texttt{OR}, \texttt{XOR}}
	\item conditionelles : \texttt{CMP}, \texttt{IF}
	\item d'accès mémoire : \texttt{LOAD}, \texttt{STORE}\\[0.7cm]
	\item \sout{... et les équivalents \textit{unsigned} : \texttt{CMPU, MULU, DIVU, MODU}}
    \end{itemize}
}
